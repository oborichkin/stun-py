...
def test_decode_stun_message_2():
    ...
    assert stunpy.is_request(message.header.message_type)
    assert message.header.method == stunpy.MessageMethod.BINDING
    ...
    assert isinstance(message.attributes[0], stunpy.Username)
    assert isinstance(message.attributes[-2], stunpy.MessageIntegrity)
    assert isinstance(message.attributes[-1], stunpy.Fingerprint)
