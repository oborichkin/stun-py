...
def test_parse_mapped_address():
    ...
    parsed_address, _ = stunpy.MappedAddress.from_bytes(data[4:])
    assert parsed_address.address_family == stunpy.AddressFamily.IPv4
...
def test_parse_xor_mapped_address():
    ...
    parsed_address, _ = stunpy.XorMappedAddress.from_bytes(data[4:])
    assert parsed_address.address_family == stunpy.AddressFamily.IPv4
...
def test_parse_username():
    ...
    parsed_username, _ = stunpy.Username.from_bytes(data[4:], 23)
...
def test_error_code():
    ...
    parsed_error, _ = stunpy.ErrorCode.from_bytes(data[4:])
